...
/* ####   This part is taken from the R package "CompQuadForm"      ##### */
...
/* ####   The above part is from the R package "CompQuadForm"      ##### */
...
List Davies (double q, NumericVector lambda, int lim = 10000, double acc = 0.0001){
    ...
    double* delta_p = delta.begin();
    IntegerVector h(p,1);
    int* h_p = h.begin();
    NumericVector trace(7);
    double* trace_p = trace.begin();
    double* lambda_p = lambda.begin();
    int* lim_p = &lim;
    double* acc_p = &acc;
    double* q_p = &q;

    qfc(lambda_p,delta_p,h_p,p_p,sigma_p,q_p,lim_p,acc_p,trace_p,ifault_p,res_p);
    List out = List::create(Named("Qq")=1-res, _["ifault"]=ifault,_["trace"]=trace);
    if (res<0){warning("Consider playing with 'lim' or 'acc'.");}
